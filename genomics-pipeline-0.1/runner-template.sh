...
COMMAND=" bash /scripts/genome_prelim_pipeline.sh "

_NUM_THREADS=auto_maxthreads
export OMP_NUM_THREADS=${numthreads:-$_NUM_THREADS}

container_exec ${CONTAINER_IMAGE} ${COMMAND} ${reads1} ${reads2} ${numthreads}
